...
ap.add_argument(
    "-s", "--save_photo", action="store_true", help="Screenshot from opencv"
)
ap.add_argument(
    "-p",
    "--take_photo",
    action="store_true",
    help="Take photos every time there is movement in the frame",
)
ap.add_argument(
    "-r",
    "--record",
    action="store_true",
    help="Start recording after the first movement",
)
ap.add_argument(
    "-t", "--tag", action="store_true", help="Hilight Tag on movements when recording"
)
...
while True:
    ...
    if cv2.countNonZero(
        diffImg(t_minus, t, t_plus)
    ) > threshold and timeCheck != datetime.now().strftime("%Ss"):
        dimg = cam.read()[1]
